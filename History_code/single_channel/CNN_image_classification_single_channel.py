...
with tf.device("/gpu:0"):
    ...
    conv1 = gen_conv_layer(X, 1, 16, layer_No=1, use_maxpool=False)
...
if __name__ == '__main__':
    RUN_TYPE = 1
    # 运行模式
    # 0 训练模式  1 测试模式  2 预测模式(指定图片路径进行预测)
    ...
    try:
        if RUN_TYPE == 0:
            ...
            select_img_to_predict("./Data/test/Water_Disaster.jpg")
            # 这个神经网络分别火灾的依据居然是有没有烟
            # 然后它还会把云当成烟
